...
df_dcc2 <- df %>% 
  mutate(
    ...
    genetics = case_when(
      str_detect(genes_plp, "MYBP") ~ "MYBPC3",
      str_detect(genes_plp, "MYH7") ~ "MYH7",
      T ~ "other"
    ),
    genetics = fct_infreq(genetics),
    genetics = fct_relevel(genetics, "MYBPC3"),
...
cox2_df <- cox2 %>%
  ...
  mutate(term = case_when(
    ...
    str_detect(term, "geneticsother") ~ "other",
    str_detect(term, "MYBP") ~ "MYBPC3",
    str_detect(term, "MYH7") ~ "MYH7",
...
plot_events<-
  df_dcc2 %>% 
  select(hcm_pub, sex, genes_plp, primary_diagnosis,event_srt, event_lvef50,t2_lvef50,t2_srt) %>%
  mutate(primary_diagnosis= "LVEF (per 5% decrease)",
         genes_plp = case_when(str_detect(genes_plp, "MYBPC3")~"MYBPC3",
                               str_detect(genes_plp, "MYH7")~"MYH7",
                               T~"other"),
  ...
  mutate(term = fct_relevel(value,"**Age at HCM diagnosis**", "12-18", "<12", "**Sex**", "Female", "Male",
                            "**Genetics**", "MYBPC3", "MYH7", "other", "Thin filament P/LP", 
                            "Multiple P/LP",
...
ggsave("fig 2 - carolyn.tiff", compression = "lzw",units = "cm", width = 20, height = 14, dpi = 1200)
...
df1<- df %>% filter(t2_d_htx_vad>echo_age0 & event_lvef50==1) %>% 
  select(pid, t2_srt, t2_d_htx_vad,event_srt, d_htx_vad_c, echo_age0,
         ...
         lvsd_timing,sex,genetics, primary_diagnosis_age,genes_plp) %>% 
  mutate(fu_comp_event = t2_d_htx_vad,
...
plot_events<-
  ...
  select(t2_lvef50, sex, genes_plp, lvef35_at_onset, event_srt ,d_htx_vad_c) %>%
  mutate(genetics = case_when(
    str_detect(genes_plp, "MYBP") ~ "MYBPC3",
    str_detect(genes_plp, "MYH7") ~ "MYH7",
    T ~ "other"),
    genetics = fct_relevel(genetics, "MYBPC3"),
         event_srt = as.character(event_srt),
         t2_lvef50 = "Per 5-year increase") %>% select(t2_lvef50, sex, genetics, lvef35_at_onset, event_srt ,d_htx_vad_c) %>% 
  pivot_longer(1:5) %>% group_by(value) %>%
...
ggsave("fig 4 - carolyn.tiff", compression = "lzw",units = "cm", width = 20, height = 14, dpi = 1200)
